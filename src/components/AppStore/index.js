...
class AppStore extends Component {
  state = {categoryTabItem: tabsList[0].tabId, AppName: ''}

  onClickTabItem = tabId => {
    this.setState({categoryTabItem: tabId})
  }
  ...
  render() {
    const {categoryTabItem, AppName} = this.state
    console.log(categoryTabItem)

    const filteredAppList = appsList.filter(
      eachApp => eachApp.category === categoryTabItem,
    )

    const filterApps = filteredAppList.filter(eachAppItem =>
      eachAppItem.appName.toLowerCase().includes(AppName.toLowerCase()),
    )
          ...
          <ul className="unOrderTabList">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                isActive={eachTab.tabId === categoryTabItem}
              />
            ))}
          </ul>
          <div>
            <ul className="unOrderAppList">
